# ========================================
# Database Module для Telegram Bot
# Версия: 1.0.0
# ========================================
...
class BotDatabase:
    """
    Встроенная база данных для логирования всех сообщений.
    
    Вместимость: 10,000 сообщений (авто-очистка старых)
    """
    ...
    def _create_tables(self):
        ...
        # Таблица сообщений (все сообщения)
        cursor.execute("""
    ...
    def _cleanup_old_messages(self):
        """Удаление старых сообщений при превышении лимита"""
        cursor = self.conn.cursor()
        ...
        if count > self.MAX_MESSAGES:
            ...
            to_delete = count - self.MAX_MESSAGES + 1000  # Оставляем запас
            cursor.execute("""
    ...
    def create_order(self, order_id: str, user_id: int, order_data: Dict, yaml_path: str):
        ...
        cursor.execute("""
            INSERT INTO orders (order_id, user_id, order_type, style, deadline, extra, status, created_at, yaml_path)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
        """, (
            order_id,
            user_id,
